	...
	for {
		...
		case internalEvent := <-eventChannel:
			...
			case StepPrepare:
				...
				if internalEvent.status.umState != pb.UpdateState_PREPARED.String() {
			...
			case StepUpdate:
				...
				if internalEvent.status.umState != pb.UpdateState_UPDATED.String() {
					t.Errorf("Unexpected UM update State  %s != %s", internalEvent.status.umState,
			...
			case StepApplyUpdate:
				...
				if internalEvent.status.umState != pb.UpdateState_IDLE.String() {
					t.Errorf("Unexpected UM update State  %s != %s", internalEvent.status.umState,
...
func TestRevert(t *testing.T) {
	...
	for {
		...
		case internalEvent := <-eventChannel:
			...
			case StepUpdate:
				if internalEvent.status.umState != pb.UpdateState_FAILED.String() {
					t.Errorf("Unexpected UM update State  %s != %s", internalEvent.status.umState,
